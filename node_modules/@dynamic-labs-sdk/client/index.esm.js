...
import { B as BaseError, a as assertDefined, g as getDefaultClient, b as getCore, I as InvalidStorageSet, c as createApiClient, e as emitEvent, s as subscribeWithSelector, i as isEqualShallow, d as dependencies, C as CannotTrackError, v as version, f as setDefaultClient, _ as _extends, r as randomString, h as getWalletProviderFromWalletAccount, j as getWalletProviderRegistry, k as getNetworkProviderBuilderRegistry } from './getNetworkProviderBuilderRegistry.esm.js';
export { l as InvalidExternalAuthError, M as MfaInvalidOtpError, m as MfaRateLimitedError, S as SandboxMaximumThresholdReachedError, q as hasExtension, o as offEvent, p as onEvent, n as onceEvent } from './getNetworkProviderBuilderRegistry.esm.js';
...
/** Completes the social redirect sign in flow. */ const completeSocialRedirectSignIn = async ({ url }, client = getDefaultClient())=>{
    const core = getCore(client);
    const apiClient = createApiClient({}, client);
    await core.initTrack.waitForAll();
...
/** Detects if the URL is a redirect from an OAuth provider. */ const detectOAuthRedirect = async ({ url }, client = getDefaultClient())=>{
    const core = getCore(client);
    const dynamicOauthState = url.searchParams.get('dynamicOauthState');
    const dynamicOauthCode = url.searchParams.get('dynamicOauthCode');
...
/** Redirects the user to the OAuth provider's authorization page */ const signInWithSocialRedirect = async ({ provider, redirectUrl, captchaToken }, client = getDefaultClient())=>{
    const core = getCore(client);
    const apiClient = createApiClient({}, client);
    await core.initTrack.waitForAll();
 ...
 */ const connectWithWalletProvider = async ({ walletProvider }, client = getDefaultClient())=>{
 ...
 */ const getWalletProviders = (client = getDefaultClient())=>{
 ...
 */ const getSdkChainFromApiChainName = (chainName)=>{
    ...
    assertDefined(chain, `chain not found for ${chainName}`);
    return chain;
};
 ...
 */ const selectPrimaryWalletAccount = async ({ walletAccount }, client = getDefaultClient())=>{
    ...
    if (walletAccount.verifiedCredentialId) {
        ...
        setUnverifiedWalletAccount({
            unverifiedWalletAccountToUpdate: _extends({}, walletAccount, {
